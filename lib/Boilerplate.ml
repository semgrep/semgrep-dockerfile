...
let map_shell_fragment (env : env) (xs : CST.shell_fragment) =
  R.List (List.map (fun x ->
    (match x with
    | `Pat_4b81dfc x -> R.Case ("Pat_4b81dfc",
        map_pat_4b81dfc env x
      )
    | `Pat_f05eb95 x -> R.Case ("Pat_f05eb95",
        map_pat_f05eb95 env x
      )
    )
  ) xs)
...
let map_unquoted_string (env : env) (xs : CST.unquoted_string) =
  R.List (List.map (fun x ->
    (match x with
    | `Imm_tok_pat_24a1611 x -> R.Case ("Imm_tok_pat_24a1611",
        map_imm_tok_pat_24a1611 env x
      )
...
let map_immediate_user_name_or_group_fragment (env : env) (x : CST.immediate_user_name_or_group_fragment) =
  (match x with
  | `Imm_tok_pat_b295287 x -> R.Case ("Imm_tok_pat_b295287",
      map_imm_tok_pat_b295287 env x
    )
  | `Imme_expa x -> R.Case ("Imme_expa",
      map_immediate_expansion env x
    )
...
let map_user_name_or_group (env : env) ((v1, v2) : CST.user_name_or_group) =
  let v1 =
    (match v1 with
    | `Pat_b295287 x -> R.Case ("Pat_b295287",
        map_pat_b295287 env x
      )
    | `Expa x -> R.Case ("Expa",
        map_expansion env x
      )
...
let map_env_pair (env : env) (x : CST.env_pair) =
  ...
  | `Env_key_imm_tok_eq_opt_choice_double_quoted_str (v1, v2, v3) -> R.Case ("Env_key_imm_tok_eq_opt_choice_double_quoted_str",
      ...
      let v3 =
        (match v3 with
        | Some x -> R.Option (Some (
            map_anon_choice_double_quoted_str_6b200ac env x
          ))
        | None -> R.Option None)
      in
      R.Tuple [v1; v2; v3]
